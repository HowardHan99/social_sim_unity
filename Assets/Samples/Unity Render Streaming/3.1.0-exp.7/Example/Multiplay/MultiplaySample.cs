...
namespace Unity.RenderStreaming.Samples
{
    class MultiplaySample : MonoBehaviour
    {
        ...
        [SerializeField] GameObject prefabHost;
        [SerializeField] GameObject prefabGuest;
        [SerializeField] GameObject prefabLocalPlayer;
        [SerializeField] GameObject menuCamera;
        [SerializeField] GameObject panel;
        [SerializeField] RawImage videoImage;
        [SerializeField] ShowStatsUI statsUI;

        // New spawn position fields
        [Header("Spawn Positions")]
        [SerializeField] Transform hostSpawnPoint;
        [SerializeField] Transform guestSpawnPoint;
        [SerializeField] Transform remotePlayerSpawnPoint; // Used for spawning guest players on the host
        ...
        void Start()
        {
            ...
            if (remotePlayerSpawnPoint == null)
                remotePlayerSpawnPoint = transform;
        }
        ...
        void SetUpHost(string username)
        {
            ...
            // Pass the remote player spawn point to the Multiplay handler
            handler.SetRemotePlayerSpawnPoint(remotePlayerSpawnPoint);

            // Spawn host player at the custom host spawn position
            var hostPlayer = GameObject.Instantiate(
                prefabLocalPlayer,
        ...
        IEnumerator SetUpGuest(string username, string connectionId)
        {
            // Spawn the guest at the custom guest spawn position
            var guestPlayer = GameObject.Instantiate(
                prefabGuest,
                guestSpawnPoint.position,
                guestSpawnPoint.rotation
            );
