...
namespace Unity.RenderStreaming.Samples
{
        ...
        IOfferHandler, IAddChannelHandler, IDisconnectHandler, IDeletedConnectionHandler
    {
        ...
        private Transform remotePlayerSpawnPoint; // Spawn point for remote players
        private List<string> connectionIds = new List<string>();
        private List<Component> streams = new List<Component>();
        private Dictionary<string, GameObject> dictObj = new Dictionary<string, GameObject>();
        ...
        // Method to set the spawn point for remote players
        public void SetRemotePlayerSpawnPoint(Transform spawnPoint)
        {
            remotePlayerSpawnPoint = spawnPoint;
        }
        ...
        public void OnOffer(SignalingEventData data)
        {
            ...
            // Use the custom spawn point if available, otherwise use default position
            Vector3 spawnPosition = remotePlayerSpawnPoint != null
                ? remotePlayerSpawnPoint.position
                : new Vector3(0, 3, 0);

            Quaternion spawnRotation = remotePlayerSpawnPoint != null
                ? remotePlayerSpawnPoint.rotation
                : Quaternion.identity;

            var newObj = Instantiate(prefab, spawnPosition, spawnRotation);
            dictObj.Add(data.connectionId, newObj);
